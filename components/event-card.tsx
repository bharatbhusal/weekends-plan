...
function TimeBadge({ start, end }: { start: Date; end?: Date }) {
  ...
  return (
    <Badge className="bg-slate-600/20 text-slate-400 border-slate-600/30 text-xs">{label}</Badge>
  );
}
      ...
      <div className="group flex gap-4 rounded-lg border bg-card p-4 shadow-sm transition-all hover:shadow-md select-none relative">
        ...
        >
          ...
          <div className="flex items-center gap-1.5 mt-1 text-xs text-muted-foreground">
            {event.eventType === "online" ? (
              ...
              <>
                <FaMapPin className="h-3 w-3 shrink-0" />
                <span className="truncate">
                  {event.location.address || event.location.city || event.location.name}
                </span>
              </>
            )}
          </div>
        ...
        <div className="flex flex-col items-center gap-2 self-start shrink-0 ml-auto">
          {onToggleWatch && (
            ...
            >
              <FaStar className={cn("h-3.5 w-3.5", isWatched && "fill-amber-500")} />
            </button>
          )}
    ...
    <div className="group relative flex flex-col rounded-lg border bg-card shadow-sm transition-all hover:shadow-md overflow-hidden animate-fade-in">
      {onToggleWatch && (
        <button
          ...
          title={isWatched ? "Remove from watchlist" : "Watch event"}
        >
          <FaStar className={cn("h-4 w-4", isWatched && "fill-amber-500")} />
        </button>
      )}
