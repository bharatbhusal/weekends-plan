...
export function extractCity(locationStr?: string): string | undefined {
  if (!locationStr) return undefined;
  const parts = locationStr.split(',').map((s) => s.trim());
  return parts[parts.length - 1];
}

export function truncateDescription(text: string, maxLen = 500): string {
...
export function computeEventAgeHours(event: NormalizedEvent): number {
  const now = Date.now();
