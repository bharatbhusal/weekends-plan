...
import { BLACKLISTED_TITLES, isBlacklisted } from "./blacklist";
...
export async function runIngestionPipeline(): Promise<IngestionResult> {
  ...
  const beforeBlacklist = allEvents.length;
  allEvents = allEvents.filter((e) => !isBlacklisted(e.title));
  if (allEvents.length < beforeBlacklist) {
    console.log(
      `Blacklist removed ${beforeBlacklist - allEvents.length} events (${allEvents.length} remaining)`,
    );
  }

  for (const event of allEvents) {
    event.location.city = generalizeCity(event.location.city) || undefined;
  }
  ...
  try {
    ...
    console.log(`Cleaned up ${pastMain.deletedCount} past events from municipal_events`);

    if (BLACKLISTED_TITLES.length > 0) {
      const blacklistRe = new RegExp(BLACKLISTED_TITLES.map(escapeRegex).join("|"), "i");
      const purged = await db.collection("municipal_events").deleteMany({ title: blacklistRe });
      if (purged.deletedCount > 0) {
        console.log(`Purged ${purged.deletedCount} blacklisted events from municipal_events`);
      }
    }

    const writeBatch = async (events: NormalizedEvent[], collectionName: string) => {
      if (events.length === 0) return { upsertedCount: 0, modifiedCount: 0 };
      const collection = db.collection<NormalizedEvent>(collectionName);
    ...
    const mainResult = await writeBatch(uniqueEvents, "municipal_events");
