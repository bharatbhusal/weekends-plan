...
function extractEventsFromApollo(apollo: Record<string, any>): MeetupRawEvent[] {
  const events: MeetupRawEvent[] = [];
  for (const [key, val] of Object.entries(apollo)) {
    if (key.startsWith("Event:") && val?.__typename === "Event" && val.title) {
...
export class MeetupClient implements Ingester {
  ...
  async fetch(config: Record<string, unknown>): Promise<NormalizedEvent[]> {
    ...
    for (const city of cities) {
      try {
        ...
        console.warn(`Meetup [${city}]: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  ...
  private async fetchCity(city: string): Promise<NormalizedEvent[]> {
  ...
  private normalize(raw: MeetupRawEvent, city: string): NormalizedEvent {
    ...
    return {
      ...
      endDateTime: raw.endTime ? new Date(raw.endTime) : undefined,
      location: {
        name: cleaned.name,
      ...
      sourceName: 'meetup',
      originalUrl: raw.eventUrl || `https://www.meetup.com/find/?keywords=tech&location=${city}`,
