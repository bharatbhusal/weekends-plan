...
export function HomePageClient({
	...
	// Drawer states
	const [selectedDayEvents, setSelectedDayEvents] = useState<{ date: Date; events: NormalizedEvent[] } | null>(null)
	...
	return (
		<div className="min-h-screen bg-background">
						...
						<div className="space-y-4">
							<div className="flex items-center justify-between flex-wrap gap-2">
								<p className="text-sm text-muted-foreground order-2 sm:order-1">
									Showing {totalVisible} of {filteredEvents.length}{" "}
									events
								</p>
								...
								{isCalendar ? (
									<div className="flex items-center justify-between gap-2 order-1 sm:order-2 w-full sm:w-auto">
										...
										<span className="text-sm font-semibold text-muted-foreground px-2 min-w-[120px] text-center">
											{new Intl.DateTimeFormat("en-IN", { month: "long", year: "numeric" }).format(currentMonth)}
										</span>
										<button
			...
			<Drawer open={!!selectedDayEvents} onOpenChange={(open) => !open && setSelectedDayEvents(null)}>
				<DrawerContent className="max-h-[85vh]">
					<div className="mx-auto w-full max-w-lg overflow-y-auto px-6 py-4">
						<DrawerHeader className="px-0">
							<DrawerTitle className="text-xl">
								Events on {selectedDayEvents ? new Intl.DateTimeFormat("en-IN", { dateStyle: "full" }).format(selectedDayEvents.date) : ""}
							</DrawerTitle>
